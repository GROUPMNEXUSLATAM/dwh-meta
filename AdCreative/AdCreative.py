...
insights = my_acconunt.get_ad_creatives(fields=fields, params=params)

df_creative = pd.DataFrame()
insights=[x for x in insights]
df_creative = pd.DataFrame(insights)

with tempfile.NamedTemporaryFile(delete=False, suffix='.csv') as tempfile:
    temp_path = tempfile.name
    print('Path del archivo: ', temp_path)
    df_creative.to_csv(temp_path, index=False)

table_id = "xaxis-latam-psa.LATAM_Datawarehouse.Creative"
